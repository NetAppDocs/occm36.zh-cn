...
* https://["支持的 Cloud Volumes ONTAP 9.4 配置"^]
* https://["ONTAP Cloud 9.3 支持的配置"^]
...
* https://["Cloud Volumes ONTAP 9.5 的存储限制"^]
* https://["Cloud Volumes ONTAP 9.4 的存储限制"^]
* https://["ONTAP Cloud 9.3 的存储限制"^]
...
--
EBS 磁盘类型:: 通用 SSD 是 Cloud Volumes ONTAP 最常见的磁盘类型。要查看 EBS 磁盘的使用情形，请参见 http://["AWS 文档： EBS 卷类型"^]。
...
虚拟机类型:: 在中查看支持的虚拟机类型 http://["《 Cloud Volumes ONTAP 发行说明》"^] 然后查看有关每个受支持 VM 类型的详细信息。请注意，每种 VM 类型都支持特定数量的数据磁盘。
+
--
* https://["Azure 文档：通用虚拟机大小"^]
* https://["Azure 文档：内存优化的虚拟机大小"^]
...
* https://["Microsoft Azure ：受管磁盘定价"^]
* https://["Microsoft Azure ：页面 Blob 定价"^]
