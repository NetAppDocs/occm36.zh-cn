...
Cloud Volumes ONTAP 通过外部密钥管理服务器支持 NetApp 卷加密。不支持板载密钥管理器。您可以在中找到支持的密钥管理器 http://["NetApp 互操作性表工具"^] 在 * 密钥管理器 * 解决方案下。
...
在 AWS 中启动 Cloud Volumes ONTAP 系统时，您可以使用启用数据加密 http://["AWS 密钥管理服务（ KMS ）"^]。Cloud Manager 使用客户主密钥（ CMK ）请求数据密钥。
...
https://["Azure 存储服务加密"^] 默认情况下， Azure 中的 Cloud Volumes ONTAP 数据会启用空闲数据。无需设置。
...
有关 Vscan 支持的供应商，软件和版本的信息，请参见 http://["NetApp 互操作性表"^]。

有关如何在 ONTAP 系统上配置和管理防病毒功能的信息，请参见 http://["《 ONTAP 9 防病毒配置指南》"^]。
