...
订阅 https://["AWS Marketplace"^] 需要接受软件条款。您只能从市场订阅。从任何位置启动 Cloud Volumes ONTAP 、但不支持 Cloud Manager 。
...
[role="quick-margin-list"]
. 转到 AWS IAM 控制台，然后通过复制和粘贴的内容来创建策略 https://["适用于 AWS 的 NetApp Cloud Central 策略"^]。
. 将策略附加到 IAM 用户。
...
[role="quick-margin-para"]
部署和管理 Cloud Volumes ONTAP 需要 Cloud Manager 软件。从启动 Cloud Manager 实例只需几分钟 https://["Cloud Central"^]。
