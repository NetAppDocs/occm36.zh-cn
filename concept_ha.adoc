...
NOTE: Cloud Manager 将创建 https://["AWS 分布放置组"^] 并启动该放置组中的两个 HA 节点。放置组通过将实例分散在不同的底层硬件上，降低同时发生故障的风险。此功能可从计算角度而不是从磁盘故障角度提高冗余。
...
image:screenshot_mount_volume_view.gif["屏幕截图：显示卷的菜单选项，其中包括挂载选项。"]
